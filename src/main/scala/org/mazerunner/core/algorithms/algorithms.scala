...
//Feb24
import org.mazerunner.core.louvain._
...
object algorithms {
  ...
  def kBetweennessCentrality(sc: SparkContext, numPartition: Int, path: String): java.lang.Iterable[String] = {
    val k = 5
    var graph= GraphLoader.edgeListFile(sc, path, 
      ...
      )partitionBy(PartitionStrategy.EdgePartition2D,12)
  ...
  /**
   * Perform a Louvain Community Detection using Spark's GraphX algorithms
   * The O(n log n) algorithm from [https://github.com/Sotera/spark-distributed-louvain-modularity]
   ...
   */
  def louvainCommunityDetection(sc: SparkContext, path:String): java.lang.Iterable[String] = {
   ...
   * @return a key-value list of calculations for each vertex
   */
  def closenessCentrality(sc: SparkContext, numPartition: Int, path: String): java.lang.Iterable[String] = {
   ...
   * @return a key-value list of calculations for each vertex
   */
  def betweennessCentrality(sc: SparkContext, numPartition: Int, path: String): java.lang.Iterable[String] = {
    // Load the edge list
    val graph = GraphLoader.edgeListFile(sc, path)
